export const SHOPPING_LISTS_TABLE = 'shoppingLists';
export const SHOPPING_LISTS_TABLE_ID = 'id';
export const SHOPPING_LISTS_TABLE_LIST_NAME = 'listName';
export const SHOPPING_LISTS_TABLE_COMPLETION_STATUS = 'completionStatus';
export const SHOPPING_LISTS_TABLE_CREATE_TIMESTAMP = 'createTimestamp';
export const SHOPPING_LISTS_TABLE_UPDATE_TIMESTAMP = 'updateTimestamp';
